...
public class ServiceControllerTest {
  ...
  void testCreateServiceSuccess() {
    // Arrange
    ServiceModel serviceModel = new ServiceModel();
    serviceModel.setName("New Service");
    ...
    ServiceModel createdService = new ServiceModel();
    createdService.setId(1);
    createdService.setName("New Service");
    createdService.setPymeId(1);
    ...
    when(pymeRepository.findByUserId(anyInt())).thenReturn(Optional.of(pyme));
    when(serviceService.createService(any(ServiceModel.class))).thenReturn(createdService);
    when(serviceController.convertToDTO(createdService)).thenReturn(serviceDTO);
  ...
  void testUpdateServiceNotFound() {
    ...
    when(serviceService.updateService(any(ServiceModel.class), anyInt())).thenReturn(null);
  ...
  void testDeleteServiceNotFound() {
    // Arrange
    doReturn(false).when(serviceService).deleteService(anyInt(), anyInt());
